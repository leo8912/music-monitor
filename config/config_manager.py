...
class ConfigManager:
    ...
    def load_from_env(self, prefix: str = "", merge: bool = True) -> Dict[str, Any]:
        ...
        for key, value in os.environ.items():
            if prefix and key.startswith(prefix):
                ...
                # 将下划线分隔的键转换为点分隔，模拟常见命名约定
                # 例如：MEDIA_DOWNLOAD_QUALITY -> media.download_quality
                # 但是要小心处理像 "DOWNLOAD_QUALITY" 这样的复合词
                # 简单的替换：将下划线替换为点
    ...
    def set(self, key: str, value: Any, validate: bool = False):
        """设置配置值"""
        # 验证值（如果定义了验证器且启用了验证）
        if validate and key in self.schema.entries:
...
def create_default_schema() -> ConfigSchema:
    ...
    # 修复验证器，使其在验证失败时抛出异常
    def validate_max_concurrent_downloads(x):
        if not (1 <= x <= 10):
            raise ValueError(f"并发下载数必须在1-10之间，当前值: {x}")
    
    schema.add_entry("media.max_concurrent_downloads", 3, "最大并发下载数", 
                     validate_max_concurrent_downloads, required=False)
    ...
    def validate_bool(x):
        if not isinstance(x, bool):
            raise ValueError(f"通知启用标志必须是布尔值，当前值: {x} (type: {type(x)})")
...
def get_config_manager() -> ConfigManager:
    ...
    """
    from core.config_manager import get_config_manager as new_get_config_manager
    return new_get_config_manager()
