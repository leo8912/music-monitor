...
测试智能元数据合并决策逻辑（纯逻辑，不依赖网络、不依赖数据库）。

来源说明
--------
本文件由 QA 从历史提交 `f7bd209^:tests/services/test_smart_merger.py` 复活。
该套用例在被删除前历史 0 失败，是性价比最高的回归资产。

复活时做的唯一改动
------------------
`test_future_default_date_is_invalid` 原本硬编码 2028 年，而
`SmartMerger.is_invalid_date` 的判定依赖 `datetime.now().year`
（`dt.year > current_year + 1` 才算无效）。这意味着这条用例在 2027 年会自动变红——
一颗定时炸弹。现改用 freezegun 冻结时间，使其结果与真实日历解耦。

Author: google (original) / music-monitor QA (revived)
Created: 2026-02-02
"""
import pytest
...
class TestSmartMergerDateValidation:
    ...
    def test_future_default_date_is_invalid(self):
        """未来年份的默认日期 (YYYY-01-01 00:00) 应判定为无效"""
        # 冻结在 2026 年：2028-01-01 00:00:00 > 当前年份+1，属于典型的"占位默认值"
        dt = datetime(2028, 1, 1, 0, 0, 0)
        assert SmartMerger.is_invalid_date(dt) is True

    @freeze_time("2026-06-01")
    def test_near_future_default_date_is_tolerated(self):
        """次年的 01-01 仍在容忍范围内（新歌预告场景），不应误杀"""
