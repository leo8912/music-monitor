...
import { getMobileMetadata } from '@/api/mobile'
...
const error = ref('')
const song = ref<MobileMetadata | null>(null)
...
const fetchMetadata = async () => {
    ...
    try {
        song.value = await getMobileMetadata({ id: String(id), sign: String(sign), expires: String(expires) })
        loading.value = false
        ...
        error.value = err.response?.data?.detail || err.message || '链接已失效或认证失败'
        loading.value = false
    }
}

// Vue Router 同路径不同 query 不会重新挂载组件,
// 监听 route.query 变化以加载新链接 (如企业微信内连续打开不同试听卡片)
watch(
    () => route.query,
    () => {
        // 重置播放器状态, 再拉取新链接元数据
...
<template>
  <div class="mobile-play-page">
    ...
    <div v-else-if="song" class="player-container">
        <!-- Background Blur -->
