...
from core.config import ensure_security_config
...
def get_secret_key() -> str:
    ...
    优先级: 内存缓存 > ConfigManager > ensure_security_config()。
    ensure_security_config 可能在启动时轮换了旧的弱密钥，
    因此当 ConfigManager 已加载时优先从中读取最新值。
    """
    global _SECRET_KEY  # noqa: PLW0603 - 密钥内存缓存单例惯用法
    if _SECRET_KEY:
        return _SECRET_KEY

    # 优先从 ConfigManager 获取 (可能已被 ensure_security_config 更新)
    try:
        from core.config_manager import get_config_manager
        s = get_config_manager().get('auth', {}).get('secret_key')
        if s and s not in ("CHANGE_ME", "default_secret_key"):
            _SECRET_KEY = s
            return _SECRET_KEY
    except Exception:
        pass

    # 回退: 从 ensure_security_config 获取
...
def reset_secret_key_cache():
    """在 ensure_security_config 轮换密钥后调用，清除缓存以便重新读取。"""
    global _SECRET_KEY
    _SECRET_KEY = None
