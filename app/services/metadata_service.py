...
class MetadataService:
    ...
    async def _simplified_search(self, title: str, artist: str) -> Optional[MetadataResult]:
        """简化搜索"""
        # 提取最核心的关键词
        main_title = title.split(maxsplit=1)[0] if title.split() else ""
        main_artist = artist.split(maxsplit=1)[0] if artist.split() else ""
    ...
    def _get_simplified_keywords(self, title: str, artist: str) -> tuple[str, str]:
        """获取简化的搜索关键词"""
        # 只取第一个词
        simple_title = title.split(maxsplit=1)[0] if title.split() else title
        simple_artist = artist.split(maxsplit=1)[0] if artist.split() else artist
