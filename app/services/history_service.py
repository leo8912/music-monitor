...
class HistoryService:
    ...
    def _format_record(self, record, matched_source: Any = None) -> Dict[str, Any]:
        ...
        if quality_val:
            try:
                ...
                if isinstance(quality_val, int):
                    if quality_val >= 999:
                        quality_label = 'SQ'  # Hi-Res/SQ
                    elif quality_val >= 320:
                        quality_label = 'HQ'
                    elif quality_val > 0:
                        quality_label = 'HQ'  # Assume HQ for other positive values as fallback
                else:
        ...
        return {
            ...
            'played_at': record.found_at.isoformat() if record.found_at else None,  # 播放时间 (兼容前端 Song.played_at)
