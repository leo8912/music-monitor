...
class SongRepository(BaseRepository[Song]):
    ...
    async def upsert_source(
        ...
        **fields,
        ...
        """
        幂等插入 SongSource (INSERT ... ON CONFLICT DO NOTHING)。

        依赖 uq_song_source 唯一约束 (alembic 迁移 a3f9c2e1d4b5 已补建)。
        相比「先查后插」, 在并发/重入场景下不会写入重复记录,
        也不会抛出 IntegrityError 破坏当前 UoW 事务。
        返回 True 表示本次实际插入, False 表示已存在 (跳过)。
        """
        from sqlalchemy.dialects.sqlite import insert as sqlite_insert

        stmt = (
            sqlite_insert(SongSource)
            .values(
                song_id=song_id,
                source=source,
            ...
            .on_conflict_do_nothing(
                index_elements=["song_id", "source", "source_id"],
            )
        )
        result = await self._session.execute(stmt)
        await self._session.flush()
        return result.rowcount > 0  # type: ignore[attr-defined]  # sqlite_insert 返回 CursorResult
    ...
    async def get_favorites(self, skip: int = 0, limit: int = 100) -> List[Song]:
        """获取收藏歌曲"""
        stmt = select(Song).options(joinedload(Song.artist)).where(Song.is_favorite).offset(skip).limit(limit)
        result = await self._session.execute(stmt)
        return result.scalars().all()
    ...
    async def get_paginated(
        ...
        if only_monitored:
            filter_stmt = filter_stmt.where(Artist.is_monitored)
