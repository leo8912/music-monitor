...
from app.schemas import MetadataFetchResponse

router = APIRouter(prefix="/api/metadata", tags=["metadata"], dependencies=[Depends(require_auth)])


@router.get("/lyrics", response_model=MetadataFetchResponse)
async def get_lyrics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取歌词失败: 服务器内部错误, 请查看日志")
...
async def get_cover(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取封面失败: 服务器内部错误, 请查看日志")


@router.post("/fetch-all", response_model=MetadataFetchResponse)
async def fetch_all_metadata(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="获取元数据失败: 服务器内部错误, 请查看日志")
