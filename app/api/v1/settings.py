...
async def get_settings():
    ...
    # 注意: 这里**不再**调用 manager.reload() ——
    #   1. reload() 是同步 DB 读 + 重写 config.yaml 的写操作, 在 async 事件循环
    #      里会阻塞事件循环 (同步 engine + Session 查询);
    #   2. 旧实现每次 GET 都新建同步 engine 且从不 dispose, SQLite 连接/句柄泄漏;
    #   3. 配置变更统一走 PATCH, PATCH 末尾已 reload_config() 更新内存,
...
async def update_settings(
    ...
    try:
        ...
        # 3. 触发配置重载 (更新内存)
        # reload 内含同步 DB 读 + config.yaml 重写, 丢到线程池避免阻塞事件循环
...
@router.post("/test-notify", response_model=GenericActionResponse)
